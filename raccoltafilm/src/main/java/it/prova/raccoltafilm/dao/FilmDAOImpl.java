...
public class FilmDAOImpl implements FilmDAO {
	...
	public List<Film> findByExample(Film example) throws Exception {
		...
		StringBuilder queryBuilder = new StringBuilder("select r from Film f where f.id = f.id ");
	...
	public List<Film> findFilmByRegista(Regista registaInput) throws Exception {
		if (registaInput == null) {
			throw new Exception("Problema valore in input");
		}
		
		TypedQuery<Film> query = entityManager.createQuery("select f from Film f where f.regista = ?1", Film.class);
		query.setParameter(1, registaInput);
